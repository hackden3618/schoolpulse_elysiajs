...
interface BulkMpesaPaymentModalProps {
  invoices: Invoice[]
  studentId: string
  studentName: string
...
export function BulkMpesaPaymentModal({
  invoices,
  studentId,
  studentName,
  guardianPhone,
  onClose,
  onSuccess,
}: BulkMpesaPaymentModalProps) {
  const { school } = useAuth()
  const [phone, setPhone] = useState("")
  const [step, setStep] = useState<Step>("input")
  const [error, setError] = useState("")

  const totalOutstanding = invoices.reduce((sum, inv) => sum + Number(inv.outstanding ?? inv.totalAmount), 0)
  const [amount, setAmount] = useState(String(totalOutstanding))
  const parsedAmount = Number(amount)
  const isValidAmount = parsedAmount > 0 && parsedAmount <= totalOutstanding
  ...
  const handleSubmit = async () => {
    ...
    // Determine allocations by fulfilling oldest/smallest invoices first (or just evenly)
    // We'll iterate through invoices and allocate the parsedAmount until it's exhausted.
    let remainingAmount = parsedAmount
    const allocations: { invoiceId: string; amount: number }[] = []

    for (const inv of invoices) {
      if (remainingAmount <= 0) break
      const invOutstanding = Number(inv.outstanding ?? inv.totalAmount)
      const allocated = Math.min(remainingAmount, invOutstanding)
        ...
        <div className="bg-gradient-to-r from-emerald-600 to-green-500 px-6 pt-6 pb-8 text-white">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              <Smartphone size={20} />
              <span className="font-semibold text-sm tracking-wide uppercase">Bulk M-Pesa Payment</span>
            </div>
          ...
          <div className="mt-1">
            <span className="text-3xl font-bold">KES {totalOutstanding.toLocaleString()}</span>
            <span className="text-emerald-200 text-sm ml-2">total outstanding</span>
          </div>
          <p className="text-emerald-50 text-xs mt-2">{invoices.length} invoices selected</p>
        </div>
          ...
          <div className="bg-white rounded-xl border border-surface-100 shadow-sm p-4 space-y-4">
              ...
              <>
                ...
                <div>
                  ...
                  <p className="text-xs text-surface-400 mt-1">
                    Amount will automatically be distributed to clear invoices.
                  </p>
                  {!isValidAmount && amount !== "" && (
                    <p className="text-xs text-danger-500 mt-1 flex items-center gap-1">
                      <AlertCircle size={12} /> Amount must be between KES 1 and {totalOutstanding.toLocaleString()}
                    </p>
                  )}
                </div>
            ...
            {step === "success" && (
              <div className="flex flex-col items-center py-6 gap-3 text-center">
                ...
                <p className="font-semibold text-surface-900">Bulk Payment Initiated</p>
