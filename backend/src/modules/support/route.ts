...
import { checkPermission, checkPlatformRole } from "@/common/middleware/permissionGuard"
...
export const schoolSupportRoute = new Elysia({
  ...
  .use(errorHandler)
  .use(authGuard)
  .guard({ beforeHandle: [checkPermission("communication:write")] }, (app) => app
...
export const platformSupportRoute = new Elysia({
  ...
  .guard({ beforeHandle: [checkPlatformRole(["super_admin", "admin", "support"])] }, (app) => app
